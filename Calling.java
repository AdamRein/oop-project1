...
public class Calling {
                ...
                for (Course course : gui.courseOfList) {
                    if (course.getName().equals(courseName)) { 
                        if (course.getStudentList().contains(student)) {
                            ...
                            student.get_course_list().add(course.getName());
                ...
                for (Course course : gui.courseOfList) {
                    if (course.getName().equals(courseName)) {
                        if (course.getInstructor() != null && course.getInstructor().equals(instructor)) {//?
                            ...
                            instructorCourseList.add(course.getName());
                            instructor.setCourseList(instructorCourseList);
                        }
                    }
                }
            }
  ...
  /*  
    public void addCourse(String courseName, String programName, List<String> preReqNames, List<String> finalPrerequisiteList) {
        for (Course course : gui.courseOfList){
            if ( course.getName().equals(courseName)){
                break;
            } else {
                for (Program program : gui.programList) {
                    if (program.getName().equals(programName)) {
                        for (String prerequisite : gui.checkPrerequisiteList){
                            if (gui.checkPrerequisiteList.contains(course.getName())) {
                                finalPrerequisiteList.add(prerequisite);
                                Program courseProgram = program;
                                createCourse(courseName, courseProgram, finalPrerequisiteList);
                            }
                        }
                    }
                }            
    ...
    public Course createCourse(String name, Program courseProgram, List<String> finalPrerequisiteList){
        Course newCourse = new Course(name, courseProgram, finalPrerequisiteList, null, null);
    ...
    */
    ...
    public void searchInstructor(String instructorName){
        for (Instructor instructor : gui.instructorList){
            if (instructor.getName().equals(instructorName)) {
...
//    public String studentCourse(String studentName, List<Student> studentList) {
//        String studentInfo = "";
